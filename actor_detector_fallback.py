...
def estimate_actors_for_scene(scene_id, frame_analysis):
    ...
    if frame_analysis['frames_with_faces'] > 0:
        ...
        try:
            ...
            likely_actors = [(actor, float(100 - i*15)) for i, actor in enumerate(sample_actors)]
...
def main():
    ...
    for movie_folder in sorted(os.listdir(SCENE_ASSETS_DIR)):
        ...
        for scene_folder in sorted(os.listdir(movie_path)):
            ...
            print(f"  Scene {scene_id}: {frame_analysis['frames_with_faces']}/{frame_analysis['total_frames']} frames with faces")
            
            # Note: Proper actor detection requires actual face embeddings matched via FAISS
            # This is a placeholder that shows the structure
            movie_results.append({
                "scene_id": scene_id,
                "characters": ["[Requires InsightFace for actual face recognition]"],
                "character_dominance_ranking": [
                    {
                        "character": "[Face recognition unavailable - protobuf conflict]",
                        "score": float(frame_analysis['frames_with_faces'] / frame_analysis['total_frames'])
                    }
                ],
        ...
        if movie_results:
            ...
            meta_path = os.path.join(OUTPUT_DIR, f"{movie_folder}_scene_actors.meta.json")
