...
def test_character_dominance_ranking_invalid_score():
    scene = {
        "scene_id": "s2",
        "movie_id": "M",
        "character_dominance_ranking": [{"character": "X", "score": 1.5}],
    }
    valid, msgs = validate_scene(scene)
    assert not valid
    assert any(
        "character_dominance_ranking" in m or "score" in m or "invalid" in m
        for m in msgs
    )
...
def test_field_confidences_structure():
    scene = {
        "scene_id": "s3",
        "movie_id": "M",
        "field_confidences": {"dialogue_text": 0.92, "objects": 0.6},
    }
    valid, msgs = validate_scene(scene)
    assert valid
    ...
    scene_bad = {
        "scene_id": "s3",
        "movie_id": "M",
        "field_confidences": {"dialogue_text": "high"},
    }
    valid, msgs = validate_scene(scene_bad)
    assert not valid
    assert any(
        "field_confidences" in m or "dialogue_text" in m or "invalid" in m for m in msgs
    )
