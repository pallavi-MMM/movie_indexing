...
def test_new_movie_with_face_actor_pipeline():
    ...
    try:
        ...
        if "field_confidences" in result:
            ...
            assert isinstance(fc, dict), f"Expected characters confidences to be dict, got {type(fc)}"
        
        print(f"✓ New movie with face->actor pipeline processed: {len(characters)} characters detected")
    finally:
...
def test_new_movie_actor_db_persistence():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(f"✓ Actor DB persisted and reloaded successfully ({len(db2.list_actors())} actors)")
