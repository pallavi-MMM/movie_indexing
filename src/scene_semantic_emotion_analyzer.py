...
def detect_laugh_moment(sentiment, arousal, dialogue_text):
    ...
    has_humor = any(
        word in (dialogue_text or "").lower()
...
def load_dialogue_for_scene(movie_name, scene_id):
    ...
    dialogue_file = os.path.join(
        SCENE_DIALOGUE_DIR, movie_name, f"{scene_id}_dialogue.json"
    )
...
def load_emotion_for_scene(movie_name, scene_id):
    ...
    emotion_file = os.path.join(
        SCENE_EMOTION_DIR, movie_name, f"{scene_id}_emotion.json"
    )
...
def analyze_scene_semantic_emotion(
    movie_name, scene_id, dialogue_data, emotion_data, visual_data=None
):
    """
    ...
    return {
        ...
        "_metadata": {
            ...
            "dialogue_length": (
                len(dialogue_data) if isinstance(dialogue_data, list) else 0
            ),
        },
    }
...
def main():
    ...
    for movie_name in sorted(movies_found):
        ...
        # Extract scenes
        scenes = (
            final_data if isinstance(final_data, list) else final_data.get("scenes", [])
        )
