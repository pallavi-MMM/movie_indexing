...
def classify_scene_category_secondary(scene):
    ...
    has_emotional_words = any(
        w in dialogue_text
        for w in ["love", "miss", "heart", "feel", "emotions", "care"]
    )
    ...
    if (
        has_emotional_words
        and has_high_arousal
        and (time_of_day == "night" or cinematic_style == "contemplative")
    ):
    ...
    if has_questions and (
        has_high_arousal or "conflict" in dialogue_text or "why" in dialogue_text
    ):
        return "suspense"
...
def add_narrative_structure(complete_schema_file):
    ...
    for scene in scenes:
        ...
        print(
            f"  [{scene_id}] {story_tag:12} | {category:20} | {func:20} | priority={priority:.3f}"
        )
        merged_count += 1
