...
class CharacterIntroductionDetector:
    ...
    def __init__(self, movie_name: str, scenes_data: Dict[str, Any], actors_data: Dict[str, Any] = None):
        """
    ...
    def detect_introductions(self) -> List[CharacterIntroduction]:
        ...
        print(f"✓ Detected {len(self.character_introductions)} unique character introductions\n")
        
        return self.character_introductions
    
    def _extract_scene_characters(self, scene: Dict[str, Any], scene_idx: int) -> List[Dict[str, Any]]:
        """
            ...
            for char in scene["characters"]:
                if isinstance(char, dict):
                    ...
                    if not char_id:
                        ...
                        char_name = char.get("name", char.get("character_name", f"Character {len(characters)}"))
                    ...
                    characters.append({
                        "character_id": char_id,
                        "character_name": char.get("name", char.get("character_name", f"Character {len(characters)}")),
        ...
        if "actors" in scene and isinstance(scene["actors"], list):
            for actor in scene["actors"]:
                if isinstance(actor, dict):
                    ...
                    if not char_id:
                        char_name = actor.get("character_name", actor.get("name", f"Character {len(characters)}"))
                        char_id = f"char_{char_name.lower().replace(' ', '_')}"
                    ...
                    characters.append({
                        "character_id": char_id,
                        "character_name": actor.get("character_name", actor.get("name", f"Character {len(characters)}")),
    ...
    def _calculate_statistics(self) -> Dict[str, Any]:
        ...
        times = [intro.introduction_time_seconds for intro in self.character_introductions]
        
        return {
            "total_introductions": len(self.character_introductions),
            "avg_introduction_time_seconds": sum(times) / len(times),
            "avg_introduction_time_formatted": self._format_time(sum(times) / len(times)),
...
class CharacterIntroductionPipeline:
    ...
    def run(self, movie_name: str, scene_file: str = None, actor_file: str = None) -> str:
        """
        ...
        # Save output
        output_path = os.path.join(self.output_dir, f"{movie_name}_character_introductions.json")
