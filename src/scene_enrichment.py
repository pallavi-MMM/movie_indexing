...
def enrich_scene_metadata(
    scene: Dict[str, Any], scene_index: int, total_scenes: int
) -> Dict[str, Any]:
    """
    ...
    if not scene.get("story_progress_tag") or scene.get("story_progress_tag") == "":
        scene["story_progress_tag"] = _compute_story_progress(
            scene_index, total_scenes, scene
        )
