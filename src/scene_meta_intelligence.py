...
STOPWORDS = {
    ...
    'of', 'with', 'by', 'from', 'is', 'are', 'am', 'was', 'were', 'be',
    'been', 'being', 'have', 'has', 'had', 'do', 'does', 'did', 'will',
    'would', 'could', 'should', 'may', 'might', 'can', 'i', 'you', 'he',
    'she', 'it', 'we', 'they', 'what', 'which', 'who', 'when', 'where',
    'why', 'how', 'that', 'this', 'these', 'those', 'as', 'if', 'not',
    'no', 'yes', 'my', 'your', 'his', 'her', 'its', 'our', 'their', 'about',
    'up', 'down', 'out', 'so', 'than', 'then', 'there', 'here', 'now', 'just',
    'only', 'over', 'under', 'me', 'him', 'them', 'us', 'tell', 'said', 'say',
    'think', 'know', 'want', 'go', 'come', 'see', 'look', 'like', 'love',
    'day', 'time', 'way', 'thing', 'one', 'two', 'three', 'cause', 'reason',
    'ok', 'yeah', 'oh', 'ah', 'well', 'dont', 'didnt', 'wont', 'cant', 'couldnt',
    'shouldnt', 'wouldnt', 'isnt', 'arent', 'wasnt', 'werent', 'havent', 'hasnt',
    'hadnt', 'doesnt', 'dont'
}
...
def classify_cinematic_style(lighting, camera_movement, motion, arousal, emotion_labels):
    """
    ...
    if arousal > 0.5 and any(label in ["sad", "angry", "tense"] for label in emotion_labels):
...
def compute_scene_priority_and_attention(scenes):
    ...
    for scene_id, metric in metrics.items():
        ...
        emotion_score = (
            (metric['arousal'] / max_arousal if max_arousal else 0) * 0.4 +
            (metric['variation'] / max_variation if max_variation else 0) * 0.3
        )
        
        # Dialogue density: count + speed
        dialogue_score = (
            (metric['dialogue_count'] / max_dialogue if max_dialogue else 0) * 0.4 +
            (metric['dialogue_speed'] / max_dialogue_speed if max_dialogue_speed else 0) * 0.1
        )
        ...
        priority = (emotion_score * 0.4 + dialogue_score * 0.4 + character_score * 0.2 + special_bonus)
...
def detect_key_plot_points(scenes, ranked_scenes):
    ...
    for scene in scenes:
        ...
        if (len(scene.get('dialogue_text', [])) > 5 and 
            scene.get('emotion_arousal_score', 0) > 0.5):
            is_key = True
            reasons.append("emotional-dialogue")
        
        if is_key:
            plot_points[scene_id] = {
                'is_key': True,
                'reasons': reasons
            }
        else:
...
def enrich_with_meta_intelligence(complete_schema):
    ...
    priority_scores, attention_scores, ranked_scenes = compute_scene_priority_and_attention(scenes)
...
def main():
    ...
    for movie_name in sorted(movies_found):
        ...
        # Save
        output_file = os.path.join(OUTPUT_JSON_DIR, f"{movie_name}_complete_schema.json")
        ...
        avg_attention = sum(s.get('viewer_attention_score', 0) for s in scenes) / len(scenes) if scenes else 0
