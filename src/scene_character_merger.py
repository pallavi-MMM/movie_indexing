...
def get_target_movie() -> str:
    ...
    movie = (
        globals().get("TARGET_MOVIE")
        or globals().get("MOVIE_NAME")
        or os.environ.get("TARGET_MOVIE")
        or os.environ.get("MOVIE_NAME")
    )
    if movie:
        return movie
    ...
    candidates = [
        os.path.basename(p).replace("_complete_schema.json", "")
        for p in glob.glob("output_json/*_complete_schema.json")
    ]
    if len(candidates) == 1:
        print(
            f"[INFO] No target movie provided; inferring movie='{candidates[0]}' from output_json"
        )
        return candidates[0]
