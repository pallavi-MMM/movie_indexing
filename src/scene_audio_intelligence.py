...
def analyze_audio_signals(audio_path):
    ...
    tempo = (
        float(np.asarray(tempo).item()) if hasattr(tempo, "__iter__") else float(tempo)
    )
...
def build_sound_design_notes(signals, peak_analysis, sfx_details, mood):
    ...
    # Mood
    notes.append(
        f"{mood} background score" if mood != "unknown" else "Neutral sound design"
    )
...
def main():
    ...
    scene_files = [
        f
        for f in os.listdir(scene_index_dir)
        if f.startswith(movie) and f.endswith("_FINAL.json")
    ]
    if not scene_files:
    ...
    for scene_file_name in sorted(os.listdir(audio_dir)):
        ...
        print(
            f"  [{scene_id}] mood={audio_intel['background_music_mood']}, "
